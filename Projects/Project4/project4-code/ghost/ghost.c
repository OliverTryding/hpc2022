...
int main(int argc, char *argv[])
{
    ...
    //  to the left
    MPI_Irecv(&data[2*DOMAINSIZE-1], 1, data_ghost, rank_right, 0, MPI_COMM_WORLD, &request);
    MPI_Send(&data[1+DOMAINSIZE], 1, data_ghost, rank_left, 0, MPI_COMM_WORLD);
    MPI_Wait(&request, &status);
    
    //  to the right
    MPI_Irecv(&data[DOMAINSIZE], 1, data_ghost, rank_left, 0, MPI_COMM_WORLD, &request);
    MPI_Send(&data[2*DOMAINSIZE-2], 1, data_ghost, rank_right, 0, MPI_COMM_WORLD);
    MPI_Wait(&request, &status);
